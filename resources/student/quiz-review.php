<?php
  ...
  if($question_result) {
    ...
    while($question_row = mysqli_fetch_array($question_result, MYSQLI_ASSOC)) {
      ...
      $data['answers'] = [];
      $answer_query = "SELECT DISTINCT(ans.id), ans.answer, ans.is_answer, qr.user_id, qr.fill_in_answer FROM answers as ans LEFT JOIN quiz_records as qr  ON qr.answer_id = ans.id AND qr.user_id = $user_id WHERE ans.question_id = $question_row[id] AND qr.quiz_id = $quiz_id ORDER BY id ASC";
