<?php
  ...
  if($question_result) {
    ...
    while($question_row = mysqli_fetch_array($question_result, MYSQLI_ASSOC)) {
      ...
      $data['answers'] = [];
      $answer_query = "SELECT DISTINCT ans.id, ans.answer, ans.is_answer, er.user_id FROM answers as ans LEFT JOIN exam_records as er  ON er.answer_id = ans.id AND er.user_id = $user_id AND er.exam_id = $exam_id WHERE ans.question_id = $question_row[id] ORDER BY id ASC";
